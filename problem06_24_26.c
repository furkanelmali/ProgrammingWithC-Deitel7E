...
int tahta[8][8][2] =
{
    ...
    {{0,4}, {0,6}, {0,8}, {0,8}, {0,8}, {0,8}, {0,6}, {0,4}},       //tahta koordinatları  ve  o koordinatın  gidebilecegi yer sayısı
...
int sayac = 0, konumX, konumY;                          // konumX konumY anlık konumu;
void Eksilt();                                      // eksilt() degisen konuma göre tahtadan tahta[][][1] hamle sayısı azaltma
void tahtagoster();
void GYDoldur(int, int);                             // gidilen yerleri  1 ile dolduruyoruz
void tahtasifirla();                                //  tekrar etmesi halinde sıfırlama
int Gidebilecegi_yerler(int, int);                   // anlık konuma göre gidebilecegi yerler
void Gkarar(int);                                    // hangi hamleyi oynucağını belirleye karar mekanizmasi
int main()
{
    int ilkKonumx, ilkKonumy;
    printf("Atin ilkKonumunu secin: ");
    scanf_s("%d %d", &ilkKonumx, &ilkKonumy);
    tahtasifirla();
    sayac = 0;
    konumX = ilkKonumx - 1;
    konumY = ilkKonumy - 1;
    ...
    while (sayac < 64)
    {
        ...
        if (GYS == 0 && sayac < 64)
        {
            printf("(%d,%d) %s \n", ilkKonumx + 1, ilkKonumy + 1, "At tahtanin her yerini gezmeyi basaramadi  \n(tekrar denemek icin t tusuna basýn: ");
            char tekrar;
            tekrar = getchar();                                                                 // BAŞARISIZ OLMA İHTİMALİNDE HEREŞEYİ SIFIRLAYIP BAŞA DÖNDÜRÜYOR.   
            if (getchar() == 't')
            {
                sayac = 0; tahtasifirla(); printf("Atin ilkKonumunu secin: "); scanf_s("%d %d", &ilkKonumx, &ilkKonumy); konumX = ilkKonumx - 1;
